...
class OpenAIResponsesProvider(LLMProvider):
    ...
    def __init__(self, config: LLMConfig):
        self.config = config
        base_url = os.environ.get(config.endpoint_env, "https://api.openai.com/v1").rstrip("/")
        self.endpoint = base_url if base_url.endswith("/responses") else f"{base_url}/responses"
        self.api_key = os.environ.get(config.api_key_env, "")
        self.model = os.environ.get(config.model_env, "gpt-5.5")
...
def extract_responses_text(raw: dict[str, Any]) -> str:
    ...
    if parts:
        return "\n".join(parts)
    raise RuntimeError(f"Unexpected OpenAI Responses payload: {raw}")
...
def build_llm_provider(config: LLMConfig) -> LLMProvider:
    ...
    if provider_name == OpenAIResponsesProvider.name:
